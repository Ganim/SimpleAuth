...
export interface ListMySessionsUseCaseRequest {
  userId: string;
}

export interface ListMySessionsUseCaseResponse {
  sessions: SessionDTO[];
}
...
export class ListMySessionsUseCase {
  ...
  async execute({
    userId,
  }: ListMySessionsUseCaseRequest): Promise<ListMySessionsUseCaseResponse> {
